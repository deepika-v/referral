                   ...
                   //check if username and password is correct
                    ...
                    {   
                      if($usr_result['0']->IsOTPVerified=="0")
                      {
                        ...
                        $text = "<a href='".base_url()."Registration/verify_otp/".$universityid."'>Click here for verify now</a>";
                      ...
                      else
                      {
                        ...
                        //$sourceid = '23';
                        //$referenceid = '4';
                        //$url = "http://ums.tnouonline.ac.in/web/checklistform.aspx";
                        $uni_name = $this->Loginmodel->get_universityname($universityid); 
                        if(stristr($uni_name['0']->LmsURL,'lms1'))
                        $url = str_replace('lms1','ums',$uni_name['0']->LmsURL);
                      else if(stristr($uni_name['0']->LmsURL,'lms'))
                        $url = str_replace('lms','ums',$uni_name['0']->LmsURL);
                      ...
                      //exit;
                        $sessiondata = array(
                              ...
                              'UniversityName'=> $uni_name['0']->UniversityName,
                              'url'=> $url."/web/checklistform.aspx",
                              'universitylink'=>$uni_name['0']->website,
                              'University'=> $uni_name['0']->University
                         );
                         //print_r($sessiondata);
                         //exit;
        ...
        if ($result!=FALSE)
         {
            ...
            $mobilenoencode =  str_replace('=','.', base64_encode($user->MobileNo));
             $url = base_url()."login/changepassword/";
                         ...
                         //print_r($status);     
                        //exit;  
            $this->session->set_flashdata('message','Link has been sent to '.$email.'. <br>Click on link to reset password' );    
            redirect('Login/forgotpassword/'.$universityid.'/');
       }
       else
         {
           $this->session->set_flashdata('message','User with email id '.$email.' and mobile no  '.$mobileno.' does not exists');    
            redirect('Login/forgotpassword/'.$universityid.'/');
     ...
     public function Check_otp(){
        ...
        if ($num_rows == '0') {
           $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Mobile number not found.</div>');
            redirect(base_url().'Login/verify_mobilenumber_view/'.$universityid);
        }else{
          ...
          if ($afftectedRows == '1')
           { 
             $_SESSION['logged_in']['IsOTPVerified'] = "1";
             $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">Mobile Number Verified Successfully</div>');
        ...
        if ($num_rows == '0') {
          ...
          if($query->IsOTPVerified=="0")
          {
           ...
           // $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">OTP sent to your registered mobile number</div>');
            //redirect(base_url().'Login/verifymobilenumber/'.$universityid);
            echo "1";
